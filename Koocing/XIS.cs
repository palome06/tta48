...
namespace Koocing
{
    public abstract partial class XI
    {
        ...
        // yard, mapping from ushort(ut) to Gamer, will yield Garden (Player) later
        public IDictionary<ushort, Gamer> Yard { set; get; }

        public Trench.Log.SvLog Log { private set; get; }

        public abstract int[] GetGameRuleCode();
        // might not be neccessary
        public abstract int AssignTeam();

        public abstract Zero.ZeroClient CreateZeroClient(ushort ut,
            Trench.VW.IWICL WI, Trench.VW.IVI VI);

        public void StartRoomSelfie()
        {
            VI = new VW.Djvi(Capacity, Log);
            VW.Djwi djwi = new VW.Djwi(Capacity, Log);
            WI = djwi;
            Yard = new Dictionary<ushort, Gamer>();
            List<VW.Neayer> neayers = new List<VW.Neayer>();
            while (neayers.Count < Capacity)
            {
                string pyName = "P"+ neayers.Count; // give tester name directly
                ushort pv = (ushort)(neayers.Count * neayers.Count);
                VW.Neayer neayer = new VW.Neayer(pyName, pv);
                neayers.Add(neayer);
            }
            ...
            {
                Gamer gamer = new Gamer(neayers[i].Name, neayers[i].Avatar, i, (ushort)uidList[i]);
                Yard.Add(gamer.Uid, gamer);
                // TODO: Board.Garden should be a mapping from uid to Gamer or Player?
            }
            Log.Start();
            foreach (Gamer gamer in Yard.Values)
            {
                Zero.ZeroClient zc = CreateZeroClient(gamer.Uid, djwi, VI);
                zc.RunAsync();
            }
            Task.Delay(100).Wait(); // are you kidding?
            WI.BCast("H0SD," + string.Join(",", Yard.Values.Select(
                p => p.Uid + "," + p.AUid + "," + p.Name)));
            levelCode = RuleCode.LEVEL_ALL;
            OpenRoom();
        }

        public void StartRoomTelephone()
        {
        }

        public void StartRoomOlympic()
        {
        }
        // following the start room procedure, mainly for opening the tunnel
        private void OpenRoom()
        {
            // Board.RoundIN = "H0PR"; // Set the Game Status
            if (WI is VW.Aywi)
                HoldRoomTunnel();
            VI.SetInGame(true);
            Run(GetGameRuleCode());
        ...
        // hold the room tunnel for new comer (reconnector and spectators)
        private void HoldRoomTunnel()
        {
            VW.Aywi aywi = WI as VW.Aywi;
            Task.Factory.StartNew(() => Execute.SafeExecute(() =>
            {
                while (true)
                {

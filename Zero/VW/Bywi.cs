...
namespace Zero.VW
{
    // TODO: check whether Ao can use it sooner
    // Server In it
    public class Bywi : Trench.VW.IWICL
    {
        ...
        private bool ConnectDo(TcpClient client, bool watch)
        {
            ...
            Trench.VW.WHelper.SentByteLine(tcpStream, watch ? ("C2QI," + Uid + "," + name) :
                ("C2CO," + Uid + "," + name + "," + avatar + "," + hopeTeam));
            while (true)
            {
                string line = Trench.VW.WHelper.ReadByteLine(tcpStream);
                if (line == null)
                    return false;
        ...
        private bool ConnectDoDirect(TcpClient client, bool watch, Trench.VW.IVI vi)
        {
            NetworkStream tcpStream = client.GetStream();
            Trench.VW.WHelper.SentByteLine(tcpStream, watch ? ("C2QI,0," + name) :
        ...
        {
            NetworkStream tcpStream = client.GetStream();
            Trench.VW.WHelper.SentByteLine(tcpStream, "C4CR," + Uid + "," + oldUid + "," + name + "," + passCode);
            while (true)
            {
                string line = Trench.VW.WHelper.ReadByteLine(tcpStream);
                if (line == null)
        ...
        public bool StartConnectDirect(bool watch, Trench.VW.IVI vi)
        {
            try
            {
                TcpClient client = new TcpClient(serverName, port);
        ...
        private void KeepOnListenRecv()
        {
            try
            {
                while (true)
                {
                    string line = Trench.VW.WHelper.ReadByteLine(stream);
        ...
        private void KeepOnListenSend()
        {
            ...
            {
                while (true)
                    Trench.VW.WHelper.SentByteLine(stream, msg0Pools.Take(ctoken.Token));
            }
            catch (IOException)
            {
                if (OnLoseConnection != null)
        ...
        internal Trench.Log.ClLog Log { set; get; }
